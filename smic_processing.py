...
data_root = 'datasets/MEGC2019/smic/HS_long/SMIC-HS-E'
smic_annotation_file = 'datasets/MEGC2019/SMIC-HS-E_annotation.csv'
...
def draw_avg_plot(features, pred_apex_idx, data, clip_name):
	...
	# frame_index_root='datasets/MEGC2019/smic/HS_long/frame index/'
	#
	# frame_index_dir = frame_index_root+clip_name
	# os.makedirs(frame_index_dir)
	dir = 'plots/{}/'.format(data)
...
def on_all_smic_clips():
	smic = pd.read_csv(smic_annotation_file)
	labels = []
	apex_frame_indices = []
	on_frame_paths = []
	off_frame_paths = []
	...
	with tqdm(total=158) as progress_bar:
		for subject, filename, on_frame_idx, off_frame_idx, emotion in samples:
			...
			print('clip_frame_paths',clip_frame_paths)
					# Find apex frame paths
			apex_frame_path, features, apex_relative_idx = find_apex_frame_of_clip(clip_frame_paths)
			print('apex_frame_path', apex_frame_path)
			print('features', features)
			print('apex_relative_idx', apex_relative_idx)
	...
	smic_data.to_csv('datasets/MEGC2019/smic_apex.csv', header=True, index=None)

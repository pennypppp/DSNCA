...
from capsule.data import data_split, sample_data, get_meta_data, MyDataset, Dataset, MyDatasetTrans , MyDatasetDual ####################################3
...
# data_root_3d = '/home/pp/DATASET/output3d5apex/casme2apex'
...
# data_four_frames_path = '/home/pp/FP_program/CAPSULE3d_dualca/datasets/samm_apex_3.csv'
# data_root = '/home/pp/DATASET'
...
def load_me_data(data_root, data_root_3d,file_path, subject_out_idx, batch_size=16, num_workers=1,is_3d=True,is_fusion=True):
	...
	if is_3d == True:
		if is_fusion == True:
			...
			train_dataset = MyDatasetDual(root=data_root,
			...
			val_dataset = MyDatasetDual(root=data_root,
										 data_root_3d=data_root_3d,
										 img_paths=val_paths,
		...
		####### RGB  transform  in 3D concat
			...
			train_dataset = MyDatasetTrans(root=data_root,
									data_root_3d=data_root_3d,
									img_paths=train_paths,
									img_labels=train_labels,
									transform= None)
			...
			val_paths, val_labels = get_meta_data(df_val)
			val_dataset = MyDatasetTrans(root=data_root,
								  data_root_3d=data_root_3d,
								  img_paths=val_paths,
								  img_labels=val_labels,
								  transform=None)
	...
	with tqdm(total=steps) as progress_bar:
		for i, (x, y) in enumerate(train_loader):  # batch training
			...
			if isinstance(x, (tuple, list)):
				first_stream = x[0].cuda()
				second_stream = x[1].cuda()
				y = y.cuda()
				optimizer.zero_grad()
				y_pred = model(first_stream,second_stream,y)
			else:
			...
			#train_loss += loss.item() * x.size(0)  # record the single batch loss  ################################################
		...
		for i, (x, y) in enumerate(test_loader):  # batch training
			...
			loss = me_loss(y, y_pred)  # compute loss
			#test_loss += loss.item() * x.size(0)  # record the batch loss  single stream loss###########################################
			test_loss += loss.item() * y.size(0)   ##dual  stream loss  ########################################################
...
def train_eval(subject_out_idx):
	...
	#model = MECapsuleNet(input_size=(42, 224, 224), classes=num_classes, routings=3, is_freeze=False)     ##################3
	model = MECapsuleNetDualCA(classes=num_classes, routings=3, is_freeze=False)
	# model = MECapsuleNetDualCAFreeze(classes=num_classes, routings=3, is_freeze=False)
	model.cuda()
...
if __name__ == '__main__':
	...
	for i in range(16):         ##### casme has 24 subjects , smic has subject 16, samm has subject28
